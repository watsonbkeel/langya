...
- [Debian] 2026-09-07 — 重机枪有效射程：`weapons.json` 的 `emplacement.type92-hmg`
  原本没有 `effectiveRangeM`，而反作弊视距校验需要它。按合理默认值补了 200m
  （与射程最远的中正式步枪一致，重机枪不应比步枪打得近）。PRD 5.2 未给出该数值，
  若与设计意图不符请告知，改配置即可，代码不用动。

- [Debian] 2026-09-07 — 反作弊限流阈值：PRD 只写了「客户端篡改本地数据无法影响计分」
  这条底线，没给具体频率上限。取值：输入 40/s（20Hz tick 的一倍余量）、
  开火 25/s（最快武器 8.3 发/s 的三倍余量）、总量 120/s、累计超限 20 次断开。
  全部写进 `gameplay.json` 的 `antiCheat` 段，并由 `verify-config.js` 交叉校验
  「不得低于 tick 率 / 不得低于最快武器射速」，防止后续改配置时把防护改成误伤。

- [Debian] 2026-09-07 — 超射程的处理方式：判为**未命中**而非拒绝射击。
  理由是射线用的是服务端权威敌人位置，作弊者伪造不了距离；射击本身
  （冷却、弹匣）是合法的，只是子弹到不了。判未命中既堵住远程作弊，
  又绝不会因为容差算错而误伤正常玩家。因此没有新增 `FireRejectReason` 枚举值，
...
- [Mac] 2026-09-06 — 鼠标上下方向：默认按 macOS 常见 Pointer Lock 行为做 Y 轴
  反向补偿，同时保留 `?invertMouseY=0/1` 作为显式覆盖入口，避免 Windows 或其他
  浏览器若表现不同时需要再改代码。理由是当前只确认 Mac 端上下反向，Windows 仍
  需要后续真实验收。

- [Mac] 2026-09-06 — 敌人清空后的波次推进：当前波次敌人被提前清空时，允许权威
  服务器提前投放下一波，不再死等原定波次时间。理由是减少阵地清场后的空窗，
  让节奏更连续，但仍由服务器控制实际投放，不让客户端自行生成敌人。

- [Mac] 2026-09-06 — M4 追加多图生成：重新生成并回填中方 `idle` / `run` /
  `fire` 三态 PNG，同时补充 `scene/mountain-backdrop` 与 `scene/supply-crate`
  两张场景图。理由是队友原有素材观感过于僵硬，场景也需要更有层次的远景与补给
  道具；当前已接入代码的场景占位节点，后续可继续微调摆放。
