...
>
> **2026-09-08 复测**：`100.74.3.56` 依然有效，局域网 `192.168.1.80` 同样可达，
> 机器已连续运行 100 天未重启。若某天「连不上 Debian」，**先 `tailscale ip -4`
> 确认地址有没有变**，不要直接判定机器掉线。

> ⚠️ **本节是服务器信息的唯一真源**。`AGENTS.md` / `PRD.md` 里出现的
> `100.126.150.80` 是历史残留，以本文件为准。上机干活前先读这一页，
> 能省掉一轮环境摸索。
